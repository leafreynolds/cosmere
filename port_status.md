...
| 11.1 | **allomancy — foundation** | `src/allomancy/java/leaf/cosmere/allomancy/common/{registries,config,eventHandlers}/`, `common/Allomancy.java` | `@Mod` ctor → `(IEventBus, ModContainer)`; all 10 `common/registries/*` (`AllomancyAttributes`, `AllomancyCreativeTabs`, `AllomancyDamageTypesRegistry`, `AllomancyEffects`, `AllomancyEntityTypes`, `AllomancyItems`, `AllomancyManifestations`, `AllomancyMenuTypes`, `AllomancyRecipes`, `AllomancyStats`); 3 config files (`AllomancyClientConfig`/`AllomancyServerConfig`/`AllomancyConfigs` — `ForgeConfigSpec` → `ModConfigSpec`, `ModLoadingContext` → injected `ModContainer`); 4 event handlers (`AllomancyCommonEvents`/`AllomancyEntityEventHandler`/`AllomancyModBusEventHandler`/`AllomancyCapabilitiesHandler` — annotation + import sweeps as in Phase 6). Mostly mechanical. |
| 11.2 | **allomancy — capabilities, networking, spiritweb submodule** | `common/capabilities/**`, `common/network/**`, `src/api/.../helpers/DrawHelper.java` | **Highest-risk sub-phase.** `ScadrialCapability` + `IScadrial` world-cap → NeoForge attachment (new pattern for the repo — no other world cap has been ported yet); `AllomancyPacketHandler` rewrite onto `BasePacketHandler`'s new shape (it still references removed `SimpleChannel`/`createChannel` — see Phase 5 notes); 2 packet classes (`EntityAllomancyActivateMessage`, `PlayerShootProjectileMessage`) → `record` + `CustomPacketPayload` + `StreamCodec`; `AllomancySpiritwebSubmodule` — its sole purpose forces restoring the real `DrawHelper` render bodies stubbed in Phase 0.8 (1.21 vertex builder rewrite: `addVertex/setColor/setNormal(Pose,…)/setUv/setLight/setOverlay`, new `RenderType.create` signature, no more `endVertex()`). `AllomancyAtium` was already partly ported in Phase 4 — finish here. |
| 11.3 | **allomancy — manifestations + gameplay items/entities/effects/coinpouch** | `common/manifestation/**`, `common/items/**`, `common/entities/**`, `common/effects/**`, `common/coinpouch/**`, `common/recipes/**`, `common/commands/**`, `common/compat/**`, `mixin/**` | 16 metal manifestation classes (`AllomancyAluminum`, `AllomancyBendalloy`, `AllomancyBrass`, `AllomancyBronze`, `AllomancyCadmium`, `AllomancyChromium`, `AllomancyCopper`, `AllomancyDuralumin`, `AllomancyElectrum`, `AllomancyGold`, `AllomancyIronSteel`, `AllomancyMalatium`, `AllomancyNicrosil`, `AllomancyPewter`, `AllomancyTin`, `AllomancyZinc`) + `AllomancyManifestation` base + `AllomancyEntityThread`; 3 items (`CoinPouchItem`, `MetalVialItem`, `MistcloakItem`); `CoinProjectile` entity; 4 effects (`AllomancyBoostEffect`, `BrassStunEffect`, `CopperCloudEffect`, `PewterBurnEffect`); CoinPouch trio (`CoinPouchContainerMenu`/`CoinPouchInventory`/`CoinPouchSlot`); `VialMixingRecipe`; `FillMetalReservesCommand` + `AllomancyCommands`; `BronzeSeekerTooltip` + `HwylaCompat` (verify Jade 1.21 surface); `EntityMixin`; `MiscHelper`. Mostly mechanical: `Holder<Attribute>`, `Holder<MobEffect>`, `DataComponents`, `AttributeModifier(ResourceLocation,…)`, Curios 9.x. |
| 11.4 | **allomancy — client + datagen** | `client/**`, `src/datagen/allomancy/**`, `src/allomancy/resources/META-INF/neoforge.mods.toml` (verify) | Client: `AllomancyKeybindings`, `AllomancyClientEvents`/`AllomancyModClientEvents`, `CoinPouchContainerScreen`, `BlockScanResult`/`IronSteelLinesThread`/`ScanResult`, `AllomancyLayerDefinitions`/`AllomancyRenderers`/`MistcloakModel`/`MistcloakRenderer`. Datagen: all 8 files (`AllomancyDataGenerator`, `AllomancyDatapackRegistryProvider`, `AllomancyEngLangGen`, `AllomancyItemModelsGen`, `AllomancyRecipeGen`, `AllomancyTagProvider`, `AllomancyPatchouliGen`, `PatchouliAllomancyCategory`) — same shape as the main-source datagen port in Phase 10 (`(PackOutput, CompletableFuture<HolderLookup.Provider>, String)` ctor, `Consumer<FinishedRecipe>` → `RecipeOutput`, `BiomeModifiers` rename, etc.). |
