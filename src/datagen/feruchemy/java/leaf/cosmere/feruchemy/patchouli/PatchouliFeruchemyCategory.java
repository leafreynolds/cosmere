...
public class PatchouliFeruchemyCategory
{
	public static void collect(List<BookStuff.Category> categories, List<BookStuff.Entry> entries)
	{
		...
		pages.add(new BookStuff.TextPage("Unfortunately, that has not yet been implemented in the mod. Instead, there should be a level of burning called Compounding and Flared Compounding, which spends allomantic charges to gain feruchemical stores quickly."));
			...
			switch (metalType)
			{
				...
				case NICROSIL:
					pages.add(new BookStuff.TextPage("A ferring who taps " + PatchouliTextFormat.Thing(metalName) + " is known as a \"" + PatchouliTextFormat.Thing(ferringName) + "\", and stores Investiture. " +
							"Little is known- more research required. (WIP)"));
					break;
				case CADMIUM:
