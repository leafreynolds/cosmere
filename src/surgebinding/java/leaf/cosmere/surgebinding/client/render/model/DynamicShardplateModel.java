...
public class DynamicShardplateModel extends HumanoidModel<LivingEntity>
{
	...
	{
		int ia = Math.round(Math.max(0f, Math.min(1f, a)) * 255f) & 0xFF;
		int ir = Math.round(Math.max(0f, Math.min(1f, r)) * 255f) & 0xFF;
		int ig = Math.round(Math.max(0f, Math.min(1f, g)) * 255f) & 0xFF;
		int ib = Math.round(Math.max(0f, Math.min(1f, b)) * 255f) & 0xFF;
	...
	public void render(ItemStack pStack, SlotContext slot, PoseStack matrixStack, MultiBufferSource buffer, int light)
	{
		...
		if (!(pStack.getItem() instanceof ShardplateCurioItem))
		{
			return;
		}
		...
		renderToBuffer(matrixStack,
				...
				packColor(color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, 1f));
...
*/
			...
			renderToBuffer(matrixStack,
					...
					LightTexture.FULL_BRIGHT,
					OverlayTexture.NO_OVERLAY,
					packColor(glyphColor.getRed() / 255f, glyphColor.getGreen() / 255f, glyphColor.getBlue() / 255f, 1f));
