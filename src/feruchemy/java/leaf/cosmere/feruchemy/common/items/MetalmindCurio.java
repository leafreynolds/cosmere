/*
 * File updated ~ 23 - 8 - 2026 ~ Leaf
 */

/*
 * File created ~ 22 - 8 - 2026 ~ Leaf
 */

package leaf.cosmere.feruchemy.common.items;

import leaf.cosmere.api.Constants;
import leaf.cosmere.api.CosmereAPI;
import leaf.cosmere.api.Metals;
import leaf.cosmere.api.manifestation.Manifestation;
import leaf.cosmere.common.cap.entity.SpiritwebCapability;
import leaf.cosmere.common.charge.IChargeable;
import net.minecraft.core.Holder;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.world.entity.ai.attributes.Attribute;
import net.minecraft.world.entity.ai.attributes.AttributeInstance;
import net.minecraft.world.item.ItemStack;
import top.theillusivec4.curios.api.SlotContext;
import top.theillusivec4.curios.api.type.capability.ICurio;

//ICurio for data-map metalminds.
//gold makes piglins neutral
//nicrosil strips powers on unequip
public class MetalmindCurio implements ICurio
{
	private final ItemStack stack;
	private final IChargeable metalmind;

	public MetalmindCurio(ItemStack stack, IChargeable metalmind)
	{
		this.stack = stack;
		this.metalmind = metalmind;
	}

	@Override
	public ItemStack getStack()
	{
		return stack;
	}

	@Override
	public boolean makesPiglinsNeutral(SlotContext slotContext)
	{
		return metalmind.getChargeMetalType(stack) == Metals.MetalType.GOLD;
	}

	@Override
	public void onUnequip(SlotContext slotContext, ItemStack newStack)
	{
		ICurio.super.onUnequip(slotContext, newStack);

		//Clear any nicrosil feruchemy based powers
		if (metalmind.getChargeMetalType(stack) == Metals.MetalType.NICROSIL)
		{
			SpiritwebCapability.get(slotContext.entity()).ifPresent(data ->
			{
				for (Manifestation manifestation : CosmereAPI.manifestationRegistry())
				{
					Holder<Attribute> attribute = manifestation.getAttribute();
					if (attribute == null)
					{
						continue;
					}

					final AttributeInstance attributeInstance = data.getLiving().getAttribute(attribute);
					if (attributeInstance != null)
					{
						attributeInstance.removeModifier(ResourceLocation.fromNamespaceAndPath("cosmere", "feru_nicrosil_" + Constants.NBT.FERU_NICROSIL_UUID));
					}
				}
			});
		}
	}
}
